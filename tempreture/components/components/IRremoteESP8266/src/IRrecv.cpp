...
static void USE_IRAM_ATTR gpio_intr() {
  ...
  timerWrite(timer, 0);
  // timerAlarmEnable(timer); //Made_Change_Here
...
void IRrecv::enableIRIn(const bool pullup) {
  ...
  // timer = timerBegin(_timer_num, 80, true); //Made_Change_Here
  ...
  // timerAlarmWrite(timer, MS_TO_USEC(params.timeout), ONCE); // Made_Change_Here
  ...
  // timerAttachInterrupt(timer, &read_timeout, false); // Made_Change_Here
...
#endif  // ESP8266
#if defined(ESP32)
  // timerAlarmDisable(timer); //Made_Change_Here
  timerEnd(timer);
#endif  // ESP32
